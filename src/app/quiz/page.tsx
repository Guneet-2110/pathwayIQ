...
export default function QuizPage() {
  ...
  function toggleMulti(option: string) {
    ...
    if (cur.includes(option)) {
      setAnswers((prev) => ({ ...prev, [q.id]: cur.filter((o: string) => o !== option) }))
    } else {
      setAnswers((prev) => ({ ...prev, [q.id]: [...cur, option] }))
    }
  }
  ...
  function toggleMultiWithNone(option: string, noneOption: string) {
    ...
    if (option === noneOption) {
      if (cur.includes(noneOption)) {
        setAnswers((prev) => ({ ...prev, [q.id]: [] }))
      } else {
        setAnswers((prev) => ({ ...prev, [q.id]: [noneOption] }))
      }
    } else {
      const withoutNone = cur.filter((o: string) => o !== noneOption)
    ...
    <main className="min-h-screen bg-gradient-to-br from-indigo-950 via-purple-950 to-slate-900 text-white flex flex-col items-center justify-center px-4 py-8">
      {showExit && (
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-50 flex items-center justify-center px-4">
          <div className="bg-slate-900 border border-white/10 rounded-2xl p-8 max-w-sm w-full text-center">
            ...
            <p className="text-white/40 text-sm mb-6">Your progress will not be saved.</p>
      ...
      <div className="w-full max-w-2xl">
        ...
        <div className="bg-white/5 border border-white/10 rounded-3xl p-8 mb-6">
          <div className="text-4xl mb-4">{(q as any).emoji}</div>
          ...
          {q.type === 'single' && (
            <div className="flex flex-col gap-3">
              {q.options!.map((option) => (
                <button key={option} onClick={() => selectSingle(option)} className={`text-left px-5 py-4 rounded-xl border transition text-sm font-medium ${answers[q.id] === option ? 'bg-indigo-500 border-indigo-400 text-white' : 'bg-white/3 border-white/10 hover:bg-white/8 hover:border-indigo-400/40 text-white/80'}`}>
                  {option}
                </button>
              ))}
            </div>
          ...
          {q.type === 'multi' && (
            <div className="flex flex-col gap-3">
              {q.options!.map((option) => (
                <button key={option} onClick={() => toggleMulti(option)} className={`text-left px-5 py-4 rounded-xl border transition text-sm font-medium flex items-center gap-3 ${(answers[q.id] || []).includes(option) ? 'bg-indigo-500 border-indigo-400 text-white' : 'bg-white/3 border-white/10 hover:bg-white/8 hover:border-indigo-400/40 text-white/80'}`}>
          ...
          {q.type === 'multi_with_none' && (
            <div className="flex flex-col gap-3">
              {q.options!.map((option) => (
                <button key={option} onClick={() => toggleMultiWithNone(option, (q as any).noneOption)} className={`text-left px-5 py-4 rounded-xl border transition text-sm font-medium flex items-center gap-3 ${(answers[q.id] || []).includes(option) ? 'bg-indigo-500 border-indigo-400 text-white' : 'bg-white/3 border-white/10 hover:bg-white/8 hover:border-indigo-400/40 text-white/80'}`}>
                  <div className={`w-4 h-4 rounded border shrink-0 flex items-center justify-center ${(answers[q.id] || []).includes(option) ? 'bg-white border-white' : 'border-white/30'}`}>
                    {(answers[q.id] || []).includes(option) && (
                      <svg className="w-2.5 h-2.5 text-indigo-600" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={3}><path strokeLinecap="round" strokeLinejoin="round" d="M5 13l4 4L19 7" /></svg>
                    )}
              ...
              <button onClick={() => toggleMultiWithNone((q as any).noneOption, (q as any).noneOption)} className={`text-left px-5 py-4 rounded-xl border transition text-sm font-medium flex items-center gap-3 ${(answers[q.id] || []).includes((q as any).noneOption) ? 'bg-white/10 border-white/30 text-white' : 'bg-white/3 border-white/10 hover:bg-white/8 text-white/50'}`}>
                <div className={`w-4 h-4 rounded border shrink-0 flex items-center justify-center ${(answers[q.id] || []).includes((q as any).noneOption) ? 'bg-white border-white' : 'border-white/30'}`}>
                  {(answers[q.id] || []).includes((q as any).noneOption) && (
                    <svg className="w-2.5 h-2.5 text-slate-600" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={3}><path strokeLinecap="round" strokeLinejoin="round" d="M5 13l4 4L19 7" /></svg>
                  )}
                </div>
                {(q as any).noneOption}
              </button>
        ...
        <div className="flex justify-between items-center">
          <button onClick={handleBack} disabled={current === 0} className="bg-white/5 hover:bg-white/10 disabled:opacity-0 text-white font-semibold px-6 py-3 rounded-xl transition">
            Back
          </button>
          <button onClick={handleNext} disabled={!isAnswered() || loading} className="bg-indigo-500 hover:bg-indigo-400 disabled:opacity-40 disabled:cursor-not-allowed text-white font-semibold px-8 py-3 rounded-xl transition">
