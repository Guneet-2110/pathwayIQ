  ...
  return (
    <div className="bg-white/5 border border-white/10 rounded-2xl p-12 text-center">
      ...
      <p className="text-white/40 text-sm mb-6 max-w-md mx-auto">
        This feature is available on the Pro plan. Upgrade to unlock Essay Brainstormer, Interview Prep, Financial Aid Estimator, and more.
      </p>
      <a href="/pricing" className="inline-block bg-indigo-500 hover:bg-indigo-400 text-white font-semibold px-6 py-3 rounded-lg transition">
    ...
    <main className="min-h-screen bg-gradient-to-br from-indigo-950 via-purple-950 to-slate-900 text-white">
      <nav className="flex items-center justify-between px-8 py-6 max-w-7xl mx-auto border-b border-white/10">
        ...
        <div className="flex gap-4 items-center">
          ...
          {!isPro && (
            <a href="/pricing" className="bg-yellow-500/20 hover:bg-yellow-500/30 text-yellow-300 text-sm font-semibold px-3 py-1.5 rounded-lg transition">⚡ Upgrade</a>
          )}
          {isPro && (
            <span className="bg-indigo-500/20 text-indigo-300 text-xs font-semibold px-3 py-1.5 rounded-lg">⚡ Pro</span>
          )}
      ...
      <div className="max-w-7xl mx-auto px-4 py-10" id="roadmap-content">
        ...
        <div className="flex gap-2 flex-wrap mb-10">
          {tabs.map((tab) => {
            const isProTab = ['Essay Ideas', 'Interview Prep', 'Financial Aid'].includes(tab)
            return (
              <button key={tab} onClick={() => setActiveTab(tab)} className={`px-4 py-2 rounded-lg text-sm font-medium transition flex items-center gap-1 ${activeTab === tab ? 'bg-indigo-500 text-white' : 'bg-white/5 text-white/50 hover:bg-white/10 hover:text-white'}`}>
                {tab}
                {isProTab && !isPro && <span className="text-yellow-400 text-xs">⚡</span>}
              </button>
            )
          })}
        </div>
        ...
        <div>
          ...
          {activeTab === 'App Timeline' && (
            <div className="flex flex-col gap-6">
              <div className="bg-indigo-500/10 border border-indigo-400/20 rounded-2xl p-4 mb-2">
                ...
                <p className="text-white/50 text-xs">Follow this checklist from junior year through senior year.</p>
              </div>
          ...
          {activeTab === 'Essay Ideas' && (isPro ? <EssayTab roadmap={roadmap} onSave={handleSaveExtra} /> : <ProGate />)}
          {activeTab === 'Interview Prep' && (isPro ? <InterviewTab career={roadmap.selected_career} roadmap={roadmap} onSave={handleSaveExtra} /> : <ProGate />)}
          {activeTab === 'Financial Aid' && (isPro ? <FinancialTab universities={plan.universities} career={roadmap.selected_career} location={plan.roadmap?.freshman?.courses?.[0] || ''} roadmap={roadmap} onSave={handleSaveExtra} /> : <ProGate />)}
        </div>
      </div>
    </main>
  )
