...
function YearTab({ data }: { data: any }) {
  if (!data) return null
  return (
    ...
    <main className="min-h-screen bg-gradient-to-br from-indigo-950 via-purple-950 to-slate-900 text-white">
      ...
      <div className="max-w-7xl mx-auto px-4 py-10">
        ...
        <div>
          ...
          {activeTab === 'Freshman' && <YearTab data={plan.roadmap?.freshman} />}
          {activeTab === 'Sophomore' && <YearTab data={plan.roadmap?.sophomore} />}
          {activeTab === 'Junior' && <YearTab data={plan.roadmap?.junior} />}
          {activeTab === 'Senior' && <YearTab data={plan.roadmap?.senior} />}
              ...
              {plan.scholarships?.map((s: any, i: number) => (
                <div key={i} className="bg-white/5 border border-white/10 rounded-2xl p-6">
                  <div className="flex justify-between items-start mb-2">
                    ...
                    <span className="bg-yellow-500/20 text-yellow-300 text-sm px-2 py-0.5 rounded-full shrink-0 ml-2">
                      {s.amount}
                    </span>
                  </div>
                  ...
                  <div className="flex justify-between text-sm">
                    ...
                    {s.link && s.link !== 'N/A' && (
                      <a href={s.link} target="_blank" rel="noopener noreferrer" className="text-indigo-400 hover:underline">
                        Apply →
                      </a>
                    )}
                  </div>
                ...
                <div key={i} className="bg-white/5 border border-white/10 rounded-2xl p-6">
                  ...
                  <div className="flex justify-between text-sm">
                    ...
                    {item.link && item.link !== 'N/A' && (
                      <a href={item.link} target="_blank" rel="noopener noreferrer" className="text-indigo-400 hover:underline">
                        Learn More →
                      </a>
                    )}
                  </div>
            ...
            <div className="grid md:grid-cols-2 gap-6">
              {plan.standardized_tests?.map((t: any, i: number) => (
                <div key={i} className="bg-white/5 border border-white/10 rounded-2xl p-6">
                  ...
                  <span className="text-white/40 text-sm">When: <span className="text-white/70">{t.when_to_take}</span></span>
                </div>
              ))}
            </div>
          )}
